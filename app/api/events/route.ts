...
type IncomingSubevent = {
  ...
  date?: string;      // falls du später doch date sendest
  description: string;
};
  ...
  try {
    ...
    if (endRaw === "" || endRaw === "0") {
      ...
      if (!Number.isFinite(endYear)) {
        return Response.json({ error: "end_year ist keine Zahl." }, { status: 400 });
      }
      incoming.set("end_year", String(endYear));
    }

    if (!title) return Response.json({ error: "title fehlt." }, { status: 400 });
    if (!startRaw) return Response.json({ error: "start_year fehlt." }, { status: 400 });

    const startYear = Number(startRaw);
    if (!Number.isFinite(startYear)) {
      return Response.json({ error: "start_year ist keine Zahl." }, { status: 400 });
    }
    ...
    try {
      era = await pb
        .collection("eras")
        .getFirstListItem(`start_year <= ${startYear} && end_year >= ${startYear}`);
    } catch (e: any) {
    ...
    if (!event?.id) {
      return Response.json({ error: "Event wurde nicht erstellt (keine id)." }, { status: 500 });
    }
    ...
    if (subeventsRaw && subeventsRaw.trim() && subeventsRaw.trim() !== "[]") {
      ...
      await Promise.all(
        subevents.map((s) => {
          ...
          const dateOrYear =
            s.date?.toString().trim() ||
            (typeof s.year === "number" && Number.isFinite(s.year) ? String(s.year) : "");

          if (!title) throw new Error("Subevent title fehlt.");
          if (!dateOrYear) throw new Error("Subevent year/date fehlt.");
